...
clinCorrel <- function(m, fileName=NULL, combat_edata=combat_edata) {
  ...
  if (nrows(corrgenes) > 20) {t <- 20} else {t <- nrows(corrgenes)}
...
clinCorrelPathways <- function(genes, fileName=NULL) {
  ...
  if (res.pathway[[2]] > 0) {
    if (!is.null(fileName)) {
      write.table(res.pathway, paste("results//", fileName, sep=""), sep="\t", row.names=T)
    }
...
clinCorrelGOs <- function(genes, fileName=NULL) {
  ...
  if (res.GO[[2]] > 0) {
  if (!is.null(fileName)) {
    write.table(res.GO, paste("results//", fileName, sep=""), sep="\t", row.names=T)
  }
...
correlGenes <- clinCorrel(meta$WUSFvol[meta$MicroarrayClass == "PSS"], "clinCorrel.WUSFvol.genes", combat_edata[rownames(res), names(meta$FS[meta$MicroarrayClass == "PSS"])])
```
...
correlGenes <- clinCorrel(meta$SSFvolL[meta$MicroarrayClass == "PSS"], "clinCorrel.SSFvolL.genes", combat_edata[rownames(res), names(meta$FS[meta$MicroarrayClass == "PSS"])])
```
...
correlGenes <- clinCorrel(meta$SSFvolR[meta$MicroarrayClass == "PSS"], "clinCorrel.SSFvolR.genes", combat_edata[rownames(res), names(meta$FS[meta$MicroarrayClass == "PSS"])])
```
...
correlGenes <- clinCorrel(meta$LGright[meta$MicroarrayClass == "PSS"], "clinCorrel.LGright.genes", combat_edata[rownames(res), names(meta$FS[meta$MicroarrayClass == "PSS"])])
```
...
correlGenes <- clinCorrel(meta$LaBioRadvalue[meta$MicroarrayClass == "PSS"], "clinCorrel.LaBioRadvalue.genes", combat_edata[rownames(res), names(meta$FS[meta$MicroarrayClass == "PSS"])])
```
...
correlGenes <- clinCorrel(meta$RoBioRadvalue[meta$MicroarrayClass == "PSS"], "clinCorrel.RoBioRadvalue.genes", combat_edata[rownames(res), names(meta$FS[meta$MicroarrayClass == "PSS"])])
```
...
correlGenes <- clinCorrel(meta$RFvalue[meta$MicroarrayClass == "PSS"], "clinCorrel.RFvalue.genes", combat_edata[rownames(res), names(meta$FS[meta$MicroarrayClass == "PSS"])])
```
...
Genes best correlating with flow counts
=====================================
```{r loadFlowData, echo=FALSE}
flow <- read.table("data//flow.txt", sep="\t", header=T, row.names=1)
...
correlGenes <- clinCorrel(flow.mtx[, "pctCD4plusRAminus.of.CD3plus."], "flowCorrel.pctCD4plusRAminus.of.CD3plus..genes", combat_edata[rownames(res), flow.patients])
```

```{r X..CD4.RA..of.CD3..._c2, dependson="RFvalue_c1"}
clinCorrelPathways(correlGenes[correlGenes[, "Pval"] < 0.05, ])
```

```{r X..CD4.RA..of.CD3..._c3, dependson="RFvalue_c1"}
clinCorrelGOs(correlGenes[correlGenes[, "Pval"] < 0.05, ])
```
...
correlGenes <- clinCorrel(flow.mtx[, "pctCD8plusRAminus.of.CD3plus."], "flowCorrel.pctCD8plusRAminus.of.CD3plus..genes", combat_edata[rownames(res), flow.patients])
```

%CD4-8-(of CD3+)
-----------------
```{r pctCD4minus8minus.of.CD3plus._c1, fig.width=12}
correlGenes <- clinCorrel(flow.mtx[, "pctCD4minus8minus.of.CD3plus."], "flowCorrel.pctCD4minus8minus.of.CD3plus..genes", combat_edata[rownames(res), flow.patients])
```

ratio.of.pctCD4plusRAminusperpct.CD8plusRAminus  
-----------------
```{r ratio.of.pctCD4plusRAminusperpct.CD8plusRAminus_c1, fig.width=12}
correlGenes <- clinCorrel(flow.mtx[, "ratio.of.pctCD4plusRAminusperpct.CD8plusRAminus"], "flowCorrel.ratio.of.pctCD4plusRAminusperpct.CD8plusRAminus.genes", combat_edata[rownames(res), flow.patients])
```
...
correlGenes <- clinCorrel(flow.mtx[idx, "pctCD38hi27hi.of.TN."], "flowCorrel.pctCD38hi27hi.of.TN..genes", combat_edata[rownames(res), flow.patients[idx]])
```
